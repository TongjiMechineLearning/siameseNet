...
def mobilenet_v1_base(inputs,
  ...
  Args:
    ...
    depth_multiplier: Float multiplier for the depth (number of channels)
      ...
      parameters or computation cost of the ckpt.
    conv_defs: A list of ConvDef namedtuples specifying the nets_bk architecture.
      ...
      for i, conv_def in enumerate(conv_defs):
        ...
        if isinstance(conv_def, Conv):
          ...
          net = slim.conv2d(net, depth(conv_def.depth), conv_def.kernel,
                            ...
                            activation_fn=None,
                            scope=end_point)
          net = tf.contrib.layers.batch_norm(net, center=True, scale=True, fused=True, scope=end_point)
          net = tf.nn.relu6(net)
        ...
        # TODO: max_pl
          ...
          net = tf.contrib.layers.batch_norm(net, center=True, scale=True, fused=True, scope=end_point)
          net = tf.nn.relu6(net)
          net = slim.max_pool2d(net, [3, 3], layer_stride, padding='SAME')
...
def mobilenet_v1(inputs,
                 ...
                 reuse=None,
                 scope='MobilenetV1'):
  """Mobilenet v1 ckpt for classification.
...
def mobilenet_v1_arg_scope(is_training=True,
  ...
  Args:
    is_training: Whether or not we're training the ckpt.
    weight_decay: The weight decay to use for regularizing the ckpt.
    stddev: The standard deviation of the trunctated normal weight initializer.
    regularize_depthwise: Whether or not apply regularization on depthwise.

  Returns:
    An `arg_scope` to use for the mobilenet v1 ckpt.
  """
  batch_norm_params = {
      'is_training': is_training,
      'center': True,
