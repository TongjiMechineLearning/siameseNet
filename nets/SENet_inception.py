...
def Global_Average_Pooling(x):
    ...
    pool_size = x.get_shape()[1].value
    stride = pool_size
    return tf.layers.average_pooling2d(inputs=x, pool_size=pool_size, strides=stride, padding="VALID")
...
class SENetResNeXt():
    def __init__(self, input, size, is_training):
    ...
    def _build_model(self):
        ...
        ##4*4*(64*size)
        net = tf.contrib.layers.conv2d(net, num_outputs=int(512*self.size),  # 生成的滤波器的数量
                                       activation_fn=None,
        ...
        # W = tf.Variable(tf.zeros([net.get_shape()[-1].value, 1024]))  # w为784*10的矩阵，输入层为784个像素点
