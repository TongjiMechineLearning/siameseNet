...
from model import siamese_loss
from nets.SENet_inception import SENetResNeXt
...
def train():
    ...
    with tf.variable_scope('input_x1') as scope:
        images = tf.placeholder(tf.float32, shape=[FLAGS.batch_size, FLAGS.image_height, FLAGS.image_width, 3])
    ...
    model = SENetResNeXt(images, 1, True)

    with tf.variable_scope('class') as scope:
        ##左支
        net = model.endpoints["conv5"]


    print(2222222222222222222222, net)
